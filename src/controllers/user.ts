...
config()
const userEmail = process.env.SENDER_MAIL as string;
const userPassword = process.env.MAIL_PWD as string;
...
export const forgetPasswordTokenSend = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const forgetPassword = async (
  ...
  try {
    ...
    if (!decoded) return next(new ErrorHandler("Invalid or expired token", 401));
